...
Db.prototype.updateGroup = function (values, callback){
    this.pool.getConnection(function(err, connection) {
        ...
        const sql = 'UPDATE `group` SET `group`.iactCD=?, `group`.setuKey=?, `group`.setu=?, `group`.seturecall=?, `group`.anti=?, `group`.individualCD=?, `group`.groupCD=?, `group`.repeat=?, `group`.antirecall=?, `group`.kouqiu=?, `group`.ban=?, `group`.blb=?, `group`.bili=? WHERE `group`.group=?';
