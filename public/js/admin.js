    ...
    success: function (result) {
        ...
        for (var i = 0; i < r.length; i++) {
            ...
            `<form id="${r[i].group}">` +
              ...
              str += `</ul></div></div><div class="form-group row"><label for="kouqiu${r[i].group}" class="col-sm-2 col-form-label">关闭小白</label><div class="col-sm-10"><input type="checkbox" class="form-control" id="kouqiu${r[i].group}"${r[i].kouqiu == 1? 'checked':''}></div></div>`+
              `<button type="submit" class="btn btn-primary">提交  QQ群：${r[i].group}</button></form></div></div></div>`;
        }
                ...
                if(rg.type == "add"){
                    i.addEventListener('click', function(event){
                        ...
                        if(!dupl && !empty) {
                            ...
                            list[g.group][list[g.group].length] = box.val();
                            ...
                            $(`#c${thisid}`).click(function() {
                                ...
                                for(i in list[ggg.grp]) {
                                    if(list[ggg.grp][i] == ggg.usr) {
                                        list[ggg.grp].pop(i);
                                        break;
                                    }
                                }
                                console.log(list[ggg.grp])
                            });
                        }
